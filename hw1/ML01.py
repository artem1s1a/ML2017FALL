...
for i in range(repeat):
    hypo = np.dot(x, w)
    err = np.ones(hypo.shape[0])*_lambda*np.mean(w)
    loss = hypo - y - err
...
_w = np.matmul(np.matmul(inv(np.matmul(x.transpose(), x) + np.identity(x.shape[1]) * _lambda), x.transpose()), y)
print(np.mean((w-_w)**2))
# save model
np.save('model.npy', w)
'''
# read model
